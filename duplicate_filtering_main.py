...
def _dedup_topk_by_sha256(scored_list, k):
    """
    Deduplicate by exact SHA-256 of 'context' while preserving order.
    ...
    for item in scored_list:
        h = hashlib.sha256(item['context'].encode('utf-8')).hexdigest()
        if h in seen:
            continue
        seen.add(h)
        deduped.append(item)
        ...
        for i in target_queries_idx:
            ...
            if args.use_truth == 'True':
                ...
                if args.attack_method not in [None, 'None']: 
                    ...
                    # NEW: deduplicate within the ranked list using SHA-256, then keep up to top_k
                    topk_results_dedup = _dedup_topk_by_sha256(topk_results, args.top_k)
                    topk_contents = [item["context"] for item in topk_results_dedup]

                    # tracking the num of adv_text in topk (after dedup)
