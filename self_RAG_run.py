...
def run(test_params):
    ...
    cmd = f"nohup python3 -u self_RAG_main.py \
        ...
        --use_selfrag True --w_rel 1.0 --w_sup 1.0 --w_use 0.5\
        --name {log_name}\
        > {log_file} &"
...
def get_log_name(test_params):
    ...
    os.makedirs(f"/mnt/SAS_A/srushti_thesis/Final_Code/PoisonedRAG/self_RAG/logs/{test_params['query_results_dir']}_logs", exist_ok=True)
    ...
    return f"/mnt/SAS_A/srushti_thesis/Final_Code/PoisonedRAG/self_RAG/logs/{test_params['query_results_dir']}_logs/{log_name}.txt", log_name
